...
class Google extends EA_Controller {
    ...
    /**
     * Complete synchronization of appointments between Google Calendar and MYAppointments.
     *
    ...
    public static function sync($provider_id = NULL)
    {
        try
        {
            ...
            foreach ($appointments as $appointment)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new Exception('Event is cancelled, remove the record from MYAppointments.');
                        }

                        // If Google Calendar event is different from MYAppointments appointment then update
                        // MYAppointments record.
                        $is_different = FALSE;
            ...
            // Add Google Calendar events that do not exist in MYAppointments.
            ...
            foreach ($google_events->getItems() as $google_event)
            {
                ...
                // Record doesn't exist in the MYAppointments, so add the event now.
                $appointment = [
     ...
     * documentation of OAuth), every MYAppointments installation should use its own calendar api key. So in every
     * api console account, the "http://path-to-MYAppointments/google/oauth_callback" should be included in an allowed redirect URL.
     */
